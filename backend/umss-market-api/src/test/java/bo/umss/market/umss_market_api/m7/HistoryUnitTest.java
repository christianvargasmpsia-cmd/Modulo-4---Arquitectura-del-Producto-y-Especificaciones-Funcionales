...
class HistoryUnitTest {
    ...
    void prepare() {
        ...
        useCase = new GetUserInteractionsSemanticUseCase(
                interactions,
                publications,
                model
        );
    ...
    void emptyHistoryReturnsNoticeWithoutReadingPublicationsOrCallingModel() {
        ...
        when(interactions.findRecentByUserId(user, 20))
                .thenReturn(List.of());

        assertEquals(
                "No tienes historial de interacciones aún.",
                useCase.executeUserHistory(user, "Mi historial")
        );

        verify(interactions)
                .findRecentByUserId(user, 20);
    ...
    void historyIncludesStoredFactsAndQuestionBeforeCallingModel() {
        ...
        when(publications.findById(publicationId))
                .thenReturn(Optional.of(publication()));

        when(model.generate(anyString()))
                .thenReturn("respuesta del doble");

        assertEquals(
                "respuesta del doble",
                useCase.executeUserHistory(
                        user,
                        "¿Qué consulté?"
                )
        );

        ArgumentCaptor<String> context =
        ...
        order.verify(interactions)
                .findRecentByUserId(user, 20);

        order.verify(publications)
                .findById(publicationId);

        order.verify(model)
                .generate(context.capture());

        assertAll(
                () -> assertTrue(
                        context.getValue().contains("Libro de álgebra")
                ),

                () -> assertTrue(
                        context.getValue().contains("[VIEW]")
                ),

                () -> assertTrue(
                        context.getValue().contains("Bs. 25.50")
                ),

                () -> assertTrue(
                        context.getValue().contains(
                                "2026-09-01T10:30"
                        )
                ),
    ...
    void removedPublicationDoesNotInventFactsInModelContext() {
        ...
        when(model.generate(anyString()))
                .thenReturn("sin detalles disponibles");
        ...
        String prompt = context.getValue();

        assertAll(
                () -> assertTrue(
                        prompt.contains("Mi historial")
                ),

                () -> assertFalse(
                        prompt.contains("Libro de álgebra")
                ),

                () -> assertFalse(
                        prompt.contains("Bs. 25.50")
                ),

                () -> assertFalse(
                        prompt.contains("Precio:")
                ),
